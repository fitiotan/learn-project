        ...
        <header class="py-2">
            <div class="container px-4 px-lg-5 my-2">
                <div class="text-center text-white">
                    <h1 class="display-4 fw-bolder">次數統計</h1>
                </div>
            </div>
        </header>
        <?php
            ...
            else
            {
                ...
                {
                    echo "<br>";
                    $sql = "SELECT DISTINCT class FROM record_resource where `account` = '".$_SESSION["account"]."'";
                    ...
                    if( mysqli_num_rows($result) > 0)
                    {
                        for( $i = 1; $i <= mysqli_num_rows($result); $i++)
                        {
                            ...
                            {
                                $class = $row["class"];
                                echo '<h4><p class="text-center">'.$class.'</p></h4><br>';
                                $sql2 = "select * from `class` where `name`='".$class."'";
                                ...
                                if( mysqli_num_rows($result2) > 0)
                                {
                                    ...
                                    {
                                        $image = $row["image"];
                                        $sql3 = "select * from `record_resource` where `class`='".$class."'";
                                        $result3 = mysqli_query($link,$sql3);
                                        if( mysqli_num_rows($result3) > 0)
                                        {
                                            echo '<section class="py-0">';
                                            echo '<div class="container px-4 px-lg-5 mt-0">';
